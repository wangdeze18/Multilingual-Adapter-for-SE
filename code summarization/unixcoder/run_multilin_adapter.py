...
def read_examples(filendir,is_train=False):
    """Read examples from filename."""
    examples = []
    if is_train:
        for lang in language:
            filename = os.path.join(filendir,lang,'train.jsonl')
        ...
        with open(filendir, encoding="utf-8") as f:
            for idx, line in enumerate(f):
                line = line.strip()
                js = json.loads(line)
                if 'idx' not in js:
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        source_tokens = [tokenizer.cls_token, "<encoder-decoder>", tokenizer.sep_token, "<mask0>"] + source_tokens + [
            tokenizer.sep_token]
...
def main():
    ...
    special_tokens_dict = {
        'additional_special_tokens': ['<java>', '<ruby>', '<go>', '<php>', '<javascript>', '<python>']}
    ...
    #delta_model = AdapterModel(backbone_model=encoder, modified_modules=['attention', '[r](\d)+\.output'],
    #                           bottleneck_dim=128)
    ...
    if args.do_train:
        train_examples = read_examples(args.train_dir, is_train=True)
        ...
        if args.local_rank == -1:
            ...
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                          batch_size=args.train_batch_size // args.gradient_accumulation_steps//4)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = []
                    for lang in language:
                        dev_filename = os.path.join(args.dev_dir, lang,'valid.jsonl')
                        examples = read_examples(dev_filename)
                        features = convert_examples_to_features(examples, tokenizer, args, stage='dev')
                        eval_examples.extend(examples)
                ...
                if dev_bleu > best_bleu:
                    ...
                    output_model_file = os.path.join(output_dir, "multilin.bin")
                    ...
                    if patience == 2:
                        break
    if args.do_test:
        checkpoint_prefix = 'checkpoint-best-bleu/multilin.bin'
        ...
        #eval_examples = []
        for lang in language:
            dev_filename = os.path.join(args.test_dir, lang, 'test.jsonl')
