...
from models import build_or_load_gen_model,build_or_load_gen_ori_model
from evaluator import smooth_bleu
from evaluator.CodeBLEU import calc_code_bleu
from evaluator.bleu import _bleu
from utils import  get_elapse_time, load_and_cache_gen_data,get_filenamelist,load_and_cache_gen_all_data,load_and_cache_gen_dev_data
...
def main():
    ...
    set_dist(args)
    set_seed(args)
    config, model, tokenizer = build_or_load_gen_ori_model(args)
    ...
    args.train_filename, args.dev_filename, args.test_filename = get_filenamelist(args.data_dir, args.task, args.sub_task)
    logger.info("  train_filename = %s", args.train_filename)
    ...
    if args.do_train:
        ...
        train_examples, train_data = load_and_cache_gen_all_data(args, args.train_filename, pool, tokenizer, 'train')
        ...
        if args.local_rank == -1:
            ...
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                          batch_size=args.train_batch_size // args.gradient_accumulation_steps // 4,num_workers=4)
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples, eval_data = load_and_cache_gen_dev_data(args, args.dev_filename, pool, tokenizer, 'dev')
                ...
                if args.save_last_checkpoints:
                    ...
                    output_model_file = os.path.join(last_output_dir, "multifull_model.bin")
                ...
                if eval_ppl < best_ppl:
                    ...
                    if args.always_save_model:
                        model_to_save = model.module if hasattr(model, 'module') else model
                        output_model_file = os.path.join(output_dir, "multifull_model.bin")
                        torch.save(model_to_save.state_dict(), output_model_file)
                ...
                if args.do_eval_bleu:
                    ...
                    if dev_bleu_em > best_bleu_em:
                        ...
                        if args.data_num == -1 or args.always_save_model:
                            model_to_save = model.module if hasattr(model, 'module') else model
                            #output_model_file = os.path.join(output_dir, "multilin_model.bin")
    ...
    if args.do_test:
        ...
        for criteria in ['best-bleu']:
            file = os.path.join(args.output_dir, 'checkpoint-{}/multifull_model.bin'.format(criteria))
            logger.info("Reload model from {}".format(file))

            model_to_load = model.module if hasattr(model,'module') else model

            model_to_load.load_state_dict(torch.load(file))
            for test_file in args.test_filename:
                eval_examples, eval_data = load_and_cache_gen_data(args, test_file, pool, tokenizer, 'test',
