...
def convert_examples_to_features_updated(examples,tokenizer,args,stage):
    features = []
    for example_index,example in enumerate(examples):
        ...
        source_ids = tokenizer.encode(source_str, max_length=args.max_source_length, padding='max_length',
                                      truncation=True)
        ...
        features.append(InputFeatures(example_index,source_ids,target_ids,url=example.url))
    return features
...
def read_summarize_examples_each(filename, data_num, sub_task):
    """Read examples from filename."""
    examples = []
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            #code = "{} {}: {}".format('summarize', sub_task, code)
