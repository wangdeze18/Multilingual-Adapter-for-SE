...
def build_or_load_gen_model(args):
    ...
    if args.model_type == 'roberta':
        ...
        #print("not apply adapter")
        delta_model = AdapterModel(backbone_model = model,modified_modules=['layer.0','layer.2','[r]encoder\.block\.(\d)+\.layer\.[01]'],bottleneck_dim=128)
        #delta_model.log(delta_ratio=True, trainable_ratio=True, visualization=True)
        #Visualization(model).structure_graph()
        #delta_model.log() 0.80%
...
def build_or_load_gen_ori_model(args):
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name)
    if args.model_type == 'roberta':
        ...
        model = model_class.from_pretrained(args.model_name_or_path)
        #print("not apply adapter")
        #delta_model = AdapterModel(backbone_model = model,modified_modules=['layer.0','layer.2','layer.1','[r]encoder\.block\.(\d)+\.layer\.[01]'],bottleneck_dim=128)
        #delta_model.log(delta_ratio=True, trainable_ratio=True, visualization=True)
        #Visualization(model).structure_graph()
        #delta_model.log() 0.80%
