...
def load_and_cache_gen_all_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    #language = [ 'ruby']
    #print("just for debug###########################only ruby")
    #for i in range(6):
    ...
    features = convert_examples_to_features_updated(examples, tokenizer, args, split_tag)
...
def load_and_cache_gen_dev_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    # cache the data into args.cache_path except it is sampled
    # only_src: control whether to return only source ids for bleu evaluating (dev/test)
    # return: examples (Example object), data (TensorDataset)
    data_tag = '_all' if args.data_num == -1 else '_%d' % args.data_num
    ...
    for file in filename:
        ...
        example = read_examples(file, args.data_num, args.task)
        #example = random.sample(example, 25)
        examples.extend(example)
    #for i in range(len(language)):
    #    fn = filename[i]
    #    subtask = language[i]
...
def get_filenamelist(data_root, task, sub_task, split=''):
    language = ['java', 'ruby','go','php','javascript','python']
    train_language = ['python','php','java']
    test_language = ['ruby','javascript','go']
    #language = [ 'ruby']
    train_fn_list =[]
    dev_fn_list = []
    test_fn_list =[]
    ...
    for lang in language:
        ...
        data_dir = '{}/{}/{}'.format(data_root, task, lang)

        train_fn = '{}/train.jsonl'.format(data_dir)
        dev_fn = '{}/valid.jsonl'.format(data_dir)
        test_fn = '{}/test.jsonl'.format(data_dir)
        train_fn_list.append(train_fn)
        dev_fn_list.append(dev_fn)
        test_fn_list.append(test_fn)

    if split == 'train':
        return train_fn_list
    elif split == 'dev':
        return dev_fn_list
    elif split == 'test':
        return test_fn_list
    else:
        return train_fn_list, dev_fn_list, test_fn_list


def get_filenamelist_down(data_root, task, sub_task, split=''):
    language = ['java', 'ruby', 'go', 'php', 'javascript', 'python']
    train_language = ['python', 'php', 'java']
    test_language = ['ruby', 'javascript', 'go']
    # language = [ 'ruby']
...
def get_filenamelist_up(data_root, task, sub_task, split=''):
    language = ['java', 'ruby', 'go', 'php', 'javascript', 'python']
    train_language = ['python', 'php', 'java']
    test_language = ['ruby', 'javascript', 'go']
    # language = [ 'ruby']
