...
def main():
    ...
    delta_model1 = AdapterModel(backbone_model=encoder, modified_modules=['attention', '[r](\d)+\.output'],bottleneck_dim=128)
    decoder_layer = nn.TransformerDecoderLayer(d_model=config.hidden_size, nhead=config.num_attention_heads)

    decoder = nn.TransformerDecoder(decoder_layer, num_layers=6)
    #delta_model2 = AdapterModel(backbone_model=decoder, modified_modules=['attention', '[r](\d)+\.output'],
    #                           bottleneck_dim=128)
    ...
    if args.do_train:
        ...
        train_examples = read_examples(args.train_filename, is_train=True)
        ...
        #    print(n)## if .decoder.
        for n, p in model.named_parameters():
            if 'adapter' in n or 'Norm' in n or 'encoder' not in n:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = []
                    for lang in language:
                        dev_filename = os.path.join(args.dev_filename, lang, 'valid.jsonl')
