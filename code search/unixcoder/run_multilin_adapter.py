...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_dir=None, istest=False,lang=None):
        self.examples = []
        data = []
        if (istest):
...
def evaluate(args, model, tokenizer,eval_data_file, lang, eval_when_training=False):
    ...
    '''
        ...
        for lang in language:
            eval_data_file = os.path.join(args.eval_data_dir, lang, 'valid.jsonl')
            codebase_name = os.path.join(args.eval_data_dir, lang, 'codebase.jsonl')
            query_data = TextDataset(tokenizer, args, eval_data_file,lang=lang)
            code_data = TextDataset(tokenizer, args, codebase_name,lang=lang)
            query_dataset.extend(query_data)
            code_dataset.extend(code_data) 
