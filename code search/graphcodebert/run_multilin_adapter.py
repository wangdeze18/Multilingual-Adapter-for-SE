...
from parser import DFG_java,DFG_ruby,DFG_javascript,DFG_php,DFG_go,DFG_python
...
def convert_examples_to_features(js,tokenizer,args,lang):
    ...
    code_tokens=code_tokens[:args.code_length+args.data_flow_length-2-min(len(dfg),args.data_flow_length)-1]
    lang_str = '<' + str(lang) + '>'
    code_tokens =[tokenizer.cls_token]+ [lang_str] + code_tokens+[tokenizer.sep_token]
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None,pool=None,is_test=False,lang=None):
        self.args=args
        self.examples = []
        self.len_list = []
        if is_test:
            ...
            for lang_i in language:
                ...
                with open(train_file) as f:
                    for line in f:
                        ...
                        lang_data.append(js)
                for js in lang_data:
                    self.examples.append(convert_examples_to_features(js,tokenizer,args,lang_i))
...
def train(args, model, tokenizer,pool):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        for lang in language:
            ...
            result = evaluate(args, model, tokenizer,eval_data_file, pool,lang,codebase_name, eval_when_training=True)
        ...
        if results['eval_mrr']>best_mrr:
            ...
            checkpoint_prefix = 'checkpoint-best-mrr/adapter'
...
def evaluate(args, model, tokenizer,file_name,pool,lang,codebase, eval_when_training=False):
    #print("file_Name",file_name)
    query_dataset = TextDataset(tokenizer, args, file_name, pool,True,lang)
    query_sampler = SequentialSampler(query_dataset)
    query_dataloader = DataLoader(query_dataset, sampler=query_sampler, batch_size=args.eval_batch_size)
    #print(args.codebase_file)
    code_dataset = TextDataset(tokenizer, args, codebase, pool,True,lang)
    code_sampler = SequentialSampler(code_dataset)
    code_dataloader = DataLoader(code_dataset, sampler=code_sampler, batch_size=args.eval_batch_size)    
...
def main():
    ...
    #nl_model = RobertaModel.from_pretrained(args.model_name_or_path)  
    #insert adapter to
    #delta_model2 = AdapterModel(backbone_model=nl_model,modified_modules=['layer.0','layer.2','[r]encoder\.block\.(\d)+\.layer[01]'],bottleneck_dim=24)
    #Visualization(nl_model).structure_graph()
    ...
    if args.do_test:
        checkpoint_prefix = 'checkpoint-best-mrr/adapter/multilin_adapter_model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir))      
        model.to(args.device)
        for lang in language:
            test_data_file = os.path.join(args.test_data_file, lang, 'test.jsonl')
            codebase_name = os.path.join(args.test_data_file, lang, 'codebase.jsonl')
            result=evaluate(args, model, tokenizer,test_data_file, pool,lang,codebase_name)
